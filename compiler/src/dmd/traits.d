...
private Dsymbol recursiveCompilesKey(TraitsExp e, Scope* sc)
{
    if (!e.args || e.args.length != 1)
        return null;

    Dsymbol resolveKey(RootObject o)
    {
...
Expression semanticTraits(TraitsExp e, Scope* sc)
{
    ...
    if (e.ident == Id.compiles)
    {
         ...
         */
        static __gshared Dsymbol[] compilesStack;
        auto key = recursiveCompilesKey(e, sc);
        if (key)
        {
            ...
            compilesStack ~= key;
            scope (exit) compilesStack = compilesStack[0 .. $ - 1];
        }
