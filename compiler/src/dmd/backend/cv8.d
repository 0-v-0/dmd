...
void cv8_func_term(Symbol* sfunc)
{
    ...
    void emitLabels(block* b)
    {
        ...
        for (; b; b = b.Bnext)
        {
            ...
            buf.write(&label32, label_v3_data.sizeof - 1);
            ...
            f1f.s = currentfuncdata.sfunc;
            f1f.offset = cast(uint)(soffset + offOffset);
            f1f.value = cast(uint)(b.Boffset - startblock.Boffset);
            currentfuncdata.f1fixup.write(&f1f, f1f.sizeof);
        }
    }

    block* labelStart = sfunc.Sfunc.Fstartblock;
    if (!labelStart)
        labelStart = bo.startblock;
    emitLabels(labelStart);
